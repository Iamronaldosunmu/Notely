...
const EditDesktopNote : React.FC<EditDesktopNoteProps> = ({notes, setNotes}) => {
    ...
    return (
        <div className="dark:bg-[#0E121A] relative h-full w-full transition-all overflow-hidden">
            ...
            <motion.input
             initial={{opacity: 0}} animate={{opacity: 1, transition: {duration: 0.15}}} exit={{opacity: 0, transition: {duration: 0.15}}}
             className="text-[30px] dark:text-white font-bold bg-transparent px-[30px] placeholder:text-[#56595F] focus:outline-[0] max-w-[100%] mb-[5px] w-full" placeholder="Add a title..." value={title} onChange={onTitleChange}
             />
            <motion.p 
                initial={{opacity: 0}} animate={{opacity: 1, transition: {duration: 0.15}}} exit={{opacity: 0, transition: {duration: 0.15}}}
