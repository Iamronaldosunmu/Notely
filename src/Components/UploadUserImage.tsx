...
interface UploadUserImageProps{
    ...
    setUser: Dispatch<SetStateAction<{_id?: string, firstName?: string, avatarUrl?: string}>>;
...
const UploadUserImage : React.FC<UploadUserImageProps> = ({setUploadImageIsShowing, uploadImageIsShowing, user, setUser, setSelectedButton, selectedButton}) => {
    ...
    return (<>
        <motion.div initial={{opacity: 0}} animate={!aboutToExit ? {opacity: 0.6, transition: {duration: 0.4}} : {opacity: 0, transition: {duration: 0.4}}}  
        onClick={() => {
            setAboutToExit(true);
            ...
            <motion.div initial={{y: 100, opacity: 0}} animate={!aboutToExit ? {y: 0, opacity: 1, transition: {duration: 0.4, ease: "easeInOut"}} : exit} className='fixed w-[300px] h-[320px] rounded-[40px] bg-white dark:bg-[#0d1117] px-7 left-[calc(50%-150px)] top-[calc(50%-160px)]'>
                ...
                <button className="w-full  rounded-xl py-2 mt-8 text-base bg-gray-200 dark:bg-[#010409] dark:text-white text-black " onClick={() => document.getElementById('getFile')?.click()}>Upload Profile Pic</button>
