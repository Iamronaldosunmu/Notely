...
const OptionsMenu : React.FC<OptionsMenuProps> = ({selectedColor, setSelectedColor, history, onDiscardButtonClick, newNote, setImageUrl, noteId}) => {
    ...
    const onFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (!newNote && e.target.files){
            ...
            const result = await axios.post('https://notelyapp1.herokuapp.com/api/v1/image', formData);
            ...
            let formData = new FormData();
            formData.append('image', e.target.files[0]);
            const result = await axios.post('https://notelyapp1.herokuapp.com/api/v1/image/newImage', formData);
            console.log(result.data);
            setImageUrl(result.data.secure_url);
        ...
        <div className={isCollapsed ? optionsContainerClasses : optionsContainerClasses + ' expanded'} >
            <div className=" flex justify-between mb-[10px]">
                ...
                <button className="ml-[14px] mr-[8px] flex items-center justify-center" onClick={() => setIsCollapsed(!isCollapsed)}>
                    {document.querySelector('html')?.classList.contains('dark') ? <img alt="icon" className={isCollapsed ? "w-[20px] expandIcon" : "w-[20px] expandIcon expanded"} src={expandIcon} /> : <img alt="icon" className={isCollapsed ? "w-[20px] expandIcon" : "w-[20px] expandIcon expanded"} src={lightModeExpandIcon} />}
                </button>
            </div>
            <div>
                <button className="flex items-center mb-[20px] w-full" onClick={onDiscardButtonClick} >
                    {document.querySelector('html')?.classList.contains('dark') ? <img alt="icon" className="mr-[30px] w-[30px] " src={whiteTrashIcon}/> : <img alt="icon" className="mr-[30px] w-[30px] " src={trashIcon}/> }

                    <p className="dark:text-[#86888C] text-[18px] font-bold">Discard Changes</p>
                </button>
                <button className="flex items-center mb-[20px] w-full">
                    {document.querySelector('html')?.classList.contains('dark') ? <img alt="icon" className="mr-[30px] w-[30px] " src={whiteUploadImageIcon}/> : <img alt="icon" className="mr-[30px] w-[30px] " src={uploadImageIcon}/> }
