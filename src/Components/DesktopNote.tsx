...
const DesktopNote : React.FC<DesktopNoteProps> = ({_id,userId, color, title, noteContent, dateCreated, imageUrl, setNoteOptionsIsShowing, setCurrentNoteId, onDesktop, removeNote}) => {
    ...
    return (
        <div className="relative transition-all w-[100%] mx-auto mb-[12px] max-w-[200px] p-[18px] bg-white dark:bg-[#151722] rounded-[25px] mt-[10px] shadow-[0_4px_10px_3px_rgba(0,0,0,0.2)] hover:scale-[0.98]">
            ...
            <DesktopNoteOptions title={title} setIsShowing={setDesktopNoteOptionsIsShowing} userId={userId} noteId={_id} removeNote={removeNote} isShowing={desktopNoteOptionsIsShowing}/>
        </div>
    );
}
