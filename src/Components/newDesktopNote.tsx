...
const NewDesktopNote : React.FC<NewDesktopNoteProps> = ({notes, setNotes}) => {
    ...
    return (
        <div className='relative h-full w-full transition-all overflow-hidden'>
            ...
            <motion.input 
                initial={{opacity: 0}} animate={{opacity: 1, transition: {duration: 0.15}}} exit={{opacity: 0, transition: {duration: 0.15}}}
                className="text-[30px] dark:text-white font-bold bg-transparent px-[30px] placeholder:text-[#56595F] focus:outline-[0] w-full max-w-[100%] mb-[5px]" placeholder="Add a title..." value={title} onChange={onTitleChange}
            />
            <motion.p 
                initial={{opacity: 0}} animate={{opacity: 1, transition: {duration: 0.15}}} exit={{opacity: 0, transition: {duration: 0.15}}}   
